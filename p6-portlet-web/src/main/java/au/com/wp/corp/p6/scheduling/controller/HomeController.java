...
public class HomeController {
	...
	public ModelAndView mediahome(HttpServletRequest request) {

		logger.debug("request attributes - is user role 'p6_portal_users_non_prod'- {} , user name- {}, session id - {} ",
				request.isUserInRole("p6_portal_users_non_prod"), request.getRemoteUser(), request.getSession().getId());
		return new ModelAndView("welcome", "userName", request.getRemoteUser());
	}
